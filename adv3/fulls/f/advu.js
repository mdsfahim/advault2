...
let statusColor = (data.status === 'approved' || data.status === 'completed') ? 'text-green-600' : (data.status === 'rejected' ? 'text-red-500' : 'text-yellow-600');
   ...
   // --- Dynamic Ad Popup with Timer & Smartlink Verification ---
    ...
    try {
        // Layer 1: Advanced IP Metadata (using ip-api.com with security fields)
        const response = await fetch('http://ip-api.com/json/?fields=status,message,country,countryCode,timezone,proxy,hosting,query');
        const data = await response.json();

        // Layer 2: System Timezone Check (The "Thinking" Layer)
        const systemTimezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
        const ipTimezone = data.timezone;

        // Layer 3: Security Flags
        const isProxyOrHosting = data.proxy === true || data.hosting === true;
        const timezoneMismatch = systemTimezone !== ipTimezone;

        console.log("--- Security Scan ---");
        console.log("IP Timezone:", ipTimezone);
        console.log("System Timezone:", systemTimezone);
        console.log("Proxy/Hosting Flag:", isProxyOrHosting);

        // Blocking Logic
        if (isProxyOrHosting || timezoneMismatch) {
            console.warn("Security Violation: VPN or Location Spoofing detected.");
            vpnDetected = true;
            handleVPNBlock();
            return true;
        }

        console.log("Security Check Passed: Connection is clean.");
        return false;

    } catch (error) {
        console.error("VPN Check Error:", error);
        // If the API fails (ad-blockers often block IP APIs), 
        // you can choose to block or allow. Usually safer to allow but log it.
    }
    return false;
}

function handleVPNBlock(isActive) {
    const wrapper = document.getElementById('earn-content-wrapper');
    const warning = document.getElementById('vpnWarning');
    const section = document.getElementById('earn-section');
    
    if (isActive) {
        // 1. Lock the content
        if (wrapper) wrapper.classList.add('earn-locked');
        
        // 2. Lock the parent section
        if (section) section.classList.add('vpn-active');
        ...
        // Haptic feedback (Telegram specific)
        if (window.Telegram?.WebApp?.HapticFeedback) {
            window.Telegram.WebApp.HapticFeedback.notificationOccurred('warning');
        }
    } else {
        // Unlock everything
        if (wrapper) wrapper.classList.remove('earn-locked');
        if (section) section.classList.remove('vpn-active');
...
function dismissToast(toast) {
    ...
    if (popup) {
        popup.style.display = 'flex'; // Triggers display
        requestAnimationFrame(() => {
            popup.classList.add('active'); // Triggers slide animation
        });
        document.body.style.overflow = 'hidden';

        // TARGETING THE NEW CLASS
        const items = popup.querySelectorAll('.guide-card-item'); 
...
// ================= PAYMENT METHODS SECTION LOGIC =================
    ...
    window.addEventListener('load', () => {
        ...
        // Run VPN Check
    const vpnFound = await checkVPN();
    if (vpnFound) {
        document.getElementById('vpnWarning')?.classList.remove('hidden');
    }
